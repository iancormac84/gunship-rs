                ...
                loop {
                    ...
                    match event {
                        TextNode(text) => {
                            ...
                            contents = Some(r#try!(parse_attrib::<String>(text)));
                        }
                ...
                loop {
                    ...
                    match event {
                        TextNode(text) => {
                            ...
                            contents = Some(r#try!(parse_attrib::<$contents_type>(text)));
                        }
                ...
                loop {
                    ...
                    match event {
                        ...
                        $(Attribute($req_attrib_str, attrib_value) => {
                            ...
                            let attrib = r#try!(parse_attrib(attrib_value));
                        ...
                        $(StartElement($req_child_str) => {
                            ...
                            let child = r#try!(parse_element(parser));
                        ...
                        $(
                            $(
                                StartElement($req_var_tag) => {
                                    ...
                                    let child: $req_var_type = r#try!(parse_element(parser));
                        ...
                        $(
                            $(
                                StartElement($opt_var_tag) => {
                                    ...
                                    let child: $opt_var_type = r#try!(parse_element(parser));
                        ...
                        $(
                            $(
                                StartElement($rep_var_tag) => {
                                    let child: $rep_var_type = r#try!(parse_element(parser));
                        ...
                        $(
                            TextNode(text) => {
                                ...
                                contents = Some(r#try!(parse_attrib::<$opt_contents_type>(text)));
                            }
                        )*
...
impl ColladaAttribute for AnyUri {
    fn parse(text: &str) -> Result<AnyUri> {
        if text.starts_with("#") {
            let uri_fragment = r#try!(parse_attrib(text));
            ...
            match event {
                ...
                StartElement("profile_COMMON") =>
                    effect.profile.push(Profile::Common(
                        r#try!(parse_element(parser)))),
                StartElement("extra") =>
                    effect.extra.push(r#try!(parse_element(parser))),
    ...
    fn parse(parser: &mut ColladaParser) -> Result<Extra> {
        ...
        loop {
            ...
            match event {
                ...
                StartElement("asset") => {
                    let asset = r#try!(parse_element(parser));
                    extra.asset = Some(asset);
                },
                StartElement("technique") => {
                    let technique = r#try!(parse_element(parser));
        ...
        loop {
            ...
            match event {
                StartElement(child) => {
                    ...
                    t = Some(r#try!(parse_element(parser)));
                },
...
impl ColladaElement for UpAxis {
    fn parse(parser: &mut ColladaParser) -> Result<UpAxis> {
        let text = r#try!(parser.parse_text_node("up_axis"));
        match text.trim() {
            "X_UP" => Ok(UpAxis::X),
            "Y_UP" => Ok(UpAxis::Y),
            "Z_UP" => Ok(UpAxis::Z),
    ...
    fn parse(&mut self) -> Result<Collada> {
        ...
        loop {
            ...
            match event {
                ...
                StartElement("asset") => {
                    let asset = r#try!(parse_element(self));
                    collada.asset = Some(asset);
                },
                StartElement("library_animations") =>
                    collada.library_animations = Some(r#try!(parse_element(self))),
                ...
                StartElement("library_effects") =>
                    collada.library_effects = Some(r#try!(parse_element(self))),
                StartElement("library_force_fields") => self.parse_library_force_fields(),
                StartElement("library_geometries") => {
                    let library_geometries = r#try!(parse_element(self));
                ...
                StartElement("library_materials") => {
                    collada.library_materials = Some(r#try!(parse_element(self)));
                },
                ...
                StartElement("library_visual_scenes") => {
                    let library_visual_scenes = r#try!(parse_element(self));
                    collada.library_visual_scenes = Some(library_visual_scenes);
                },
                StartElement("scene") =>
                    collada.scene = Some(r#try!(parse_element(self))),
                StartElement("extra") => {
                    let extra = r#try!(parse_element(self));
