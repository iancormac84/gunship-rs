...
impl Window {
    pub fn new(_name: &str, _instance: ()) -> Rc<RefCell<Window>> {
        unsafe {
            ...
            frame_attributes.event_mask = xlib::KeyPressMask
                | xlib::KeyReleaseMask
            ...
            xlib::XStoreName(
                display,
                window,
                mem::transmute(CString::new("Gunship Game").unwrap().as_ptr()),
            );
            ...
            while xlib::XPending(self.display) > 0 {
                ...
                match event.get_type() {
                    xlib::KeyPress => {
                        ...
                        let mut num_syms = 0;
                        let ptr_key_sym = xlib::XGetKeyboardMapping(
                            self.display,
                            key_press_event.keycode as u8,
                            1,
                            &mut num_syms,
                        );
                    ...
                    xlib::KeyRelease => {
                        ...
                        let ptr_key_sym = xlib::XGetKeyboardMapping(
                            self.display,
                            key_release_event.keycode as u8,
                            1,
                            &mut num_syms,
                        );
                        let syms_slice = slice::from_raw_parts(ptr_key_sym, num_syms as usize);
                    ...
                    xlib::MotionNotify => {
                        let pointer_motion_event: &xlib::XPointerMovedEvent =
                            mem::transmute(&event);
